...
| `PART_OF`           | `Location`    | `Country`/`State` | Represents the hierarchical structure of locations (e.g., City -> State -> Country). The depth of this hierarchy is determined by the metadata available in Synology Photos. |
...
_Note: This schema is maintained by the `synofoto-graph-sync` tool, which populates the graph from Synology Photos metadata._
